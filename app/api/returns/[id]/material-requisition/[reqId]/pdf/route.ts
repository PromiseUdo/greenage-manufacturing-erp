...
export async function GET(
  ...
  try {
    ...
    doc.text('REPAIR MATERIAL REQUISITION', 15, 22);

    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`Ref: ${requisition.requisitionNumber}`, 15, 31);
    doc.text(
      `Date: ${new Date(requisition.createdAt).toLocaleDateString('en-GB', {
        day: '2-digit',
        month: 'short',
        year: 'numeric',
      })}`,
    ...
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.text('Returns & Repair Division', 195, 30, { align: 'right' });
    ...
    if (ret?.issueReported) {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...accentColor);
      doc.text('ISSUE REPORTED', col1X, infoY + 54);
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(...primaryColor);
      doc.text(issueLines, col1X, infoY + 60);
    }

    const cleanNotes = requisition.notes?.replace(/^\[.*?\]\s*/, '') ?? '';
    if (cleanNotes) {
      drawInfoRow('Notes', cleanNotes, col1X, infoY + 74);
    }

    // === Divider ===
    const tableStartY = infoY + 90;
