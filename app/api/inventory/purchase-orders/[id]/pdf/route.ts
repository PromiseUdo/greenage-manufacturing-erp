...
export async function GET(
  ...
  try {
    ...
    if (supplier.contactPerson) doc.text(`Attn: ${supplier.contactPerson}`, 15, 71);
    ...
    autoTable(doc, {
      startY: 98,
      head: [['#', 'Material', 'Part No.', 'Unit', 'Qty', 'Unit Cost', 'Total']],
      body: tableData,
      headStyles: {
    ...
    const totalY = finalY + (purchaseOrder.tax > 0 ? 7 : 0) + (purchaseOrder.discount > 0 ? 7 : 0) + 7;
