...
// src/app/api/inventory/tools/lending/[id]/return/route.ts - UPDATED
  ...
  try {
    ...
    const lending = await prisma.toolLending.findUnique({
      ...
      include: {
        tool: {
          include: {
            toolGroup: true, // Include group if it exists
          },
        },
      },
    });
    ...
    // Update lending record and tool status in transaction
    const [updatedLending] = await prisma.$transaction(async (tx) => {
      // Update lending record
      const lending = await tx.toolLending.update({
        where: { id },
        data: {
          returnedAt: new Date(),
          returnedTo: session.user.name!,
        ...
        include: { tool: { include: { toolGroup: true } } },
      });
      ...
      // If tool belongs to a group, increment available quantity
      if (lending.tool.toolGroupId && newToolStatus === 'AVAILABLE') {
        await tx.toolGroup.update({
          where: { id: lending.tool.toolGroupId },
          data: {
            availableQuantity: {
              increment: 1,
            },
    ...
    await prisma.activityLog.create({
      data: {
        ...
        details: {
          toolId: lending.toolId,
          toolName: lending.tool.name,
          toolIdentifier: lending.tool.toolId, // Use toolId instead of toolNumber
          returnedFrom: lending.issuedTo,
          returnCondition,
          newStatus: newToolStatus,
        },
