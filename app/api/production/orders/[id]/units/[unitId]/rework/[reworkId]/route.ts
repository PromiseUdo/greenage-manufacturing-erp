  ...
  try {
    ...
    if (action === 'COMPLETE') {
      ...
      await prisma.unitRework.update({
        where: { id: reworkId },
        data: { status: 'COMPLETED', completedAt: new Date(), completedById: session.user.id, notes: notes || rework.notes },
      });
    ...
    return NextResponse.json({ error: 'Failed to update rework' }, { status: 500 });
  }
}
