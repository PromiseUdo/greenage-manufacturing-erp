...
type Params = { params: Promise<{ id: string; unitId: string; reworkId: string }> };
...
export async function POST(request: NextRequest, { params }: Params) {
  try {
    ...
    const rework = await prisma.unitRework.findUnique({
      where: { id: reworkId },
      select: { id: true, productionUnitId: true, status: true, _count: { select: { tasks: true } } },
    });
    ...
    if (rework.status !== 'IN_PROGRESS') {
      return NextResponse.json({ error: 'Cannot add tasks to a completed or cancelled rework' }, { status: 400 });
    }
