            ...
            <Box key={i} sx={{ position: 'relative' }}>
              {f.mimeType.startsWith('image/') ? (
                <Box
                  ...
                  alt={f.name}
                  onClick={() =>
                    onPreview(`data:${f.mimeType};base64,${f.base64}`)
                  }
                  sx={{
                    width: 80,
                    height: 80,
...
export default function ProductionReturnDetailClient({
  ...
  // ── Return status ──────────────────────────────────────────────
  const [returnStatus, setReturnStatus] = useState<string>(initialData.status);

  // ── Units ──────────────────────────────────────────────────────
  const [units, setUnits] = useState<ReturnUnit[]>(
    initialData.returnUnits ?? [],
  );
  ...
  const allUnitsRepairDone =
    units.length > 0 &&
    units.every(
      (u) => u.status === 'REPAIR_COMPLETED' || u.status === 'SCRAPPED',
    ) &&
    units.some((u) => u.status === 'REPAIR_COMPLETED');
      ...
      if (res.ok) {
        ...
        setUnits((prev) =>
          prev.map((u) => {
            ...
            const newStatus =
              unitStatus && u.repairTasks.some((t) => t.id === updated.id)
                ? unitStatus
                : u.status;
  ...
  const approveRepairComplete = async () => {
    setApprovingRepair(true);
    try {
      const res = await fetch(`/api/returns/${data.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: 'REPAIR_COMPLETED' }),
      });
      if (res.ok) {
        setReturnStatus('REPAIR_COMPLETED');
      } else {
        const err = await res.json();
        alert(err.error ?? 'Failed to approve repair');
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const addReqItem = (material: any) => {
    setReqItems((prev) => [...prev, { material, quantityRequired: 1 }]);
  };
  ...
  return (
    <Box sx={{ maxWidth: 1100, mx: 'auto' }}>
      ...
      <Paper
        elevation={0}
        sx={{ p: 3, borderRadius: 3, border: '1px solid #E5E7EB', mb: 3 }}
      >
        <Grid container spacing={3}>
          ...
          <Grid size={{ xs: 12 }}>
            ...
            <Box sx={{ mt: 1.5 }}>
              ...
              <Typography
                variant="body2"
                sx={{ p: 1.5, bgcolor: '#F9FAFB', borderRadius: 2, mt: 0.5 }}
              >
                {data.issueReported}
              </Typography>
              {data.condition && (
      ...
      {activeUnit && (
        <Box>
          ...
          {isScrapped && (
            ...
            >
              This unit has been marked as scrapped and will not proceed to
              repair.
            </Alert>
          )}
          ...
          {/* ── Stage 2: Inspection ───────────────────────────── */}
          {(STATUS_ORDER[activeUnit.status] >= 1 ||
            activeUnit.status === 'SCRAPPED') && (
            <StageCard
              stageNum={2}
              title="Inspection"
              icon=""
              ...
              {activeUnit.status === 'INSPECTING' ? (
                <>
                  <Alert severity="info" sx={{ mb: 2, borderRadius: 2 }}>
                    Inspector: document your findings below and submit for
                    manager approval.
                  </Alert>
                  <Button
                    variant="contained"
                    size="small"
                    onClick={() =>
                      openStageDialog('submit_findings', activeUnit.id)
                    }
                    sx={{
                      bgcolor: '#4338CA',
                ...
                <>
                  ...
                  >
                    by {activeUnit.inspectedBy?.name} ·{' '}
                    {fmtDate(activeUnit.inspectedAt)}
                  </Typography>
                </>
              ) : (
                <Typography variant="body2" color="text.secondary">
          ...
          {(STATUS_ORDER[activeUnit.status] >= 2 ||
            activeUnit.status === 'SCRAPPED') && (
            <StageCard
              stageNum={3}
              ...
              {activeUnit.status === 'PENDING_APPROVAL' ? (
                <>
                  <Alert severity="warning" sx={{ mb: 2, borderRadius: 2 }}>
                    Awaiting manager decision — approve for repair or scrap this
                    unit.
                  </Alert>
                ...
                <>
                  ...
                  >
                    ...
                    <Typography
                      ...
                      sx={{ mt: 1, display: 'block' }}
                    >
                      by {activeUnit.approvedBy?.name} ·{' '}
                      {fmtDate(activeUnit.approvedAt)}
                    </Typography>
                  </Box>
                </>
              ) : null}
                        ...
                        >
                          ...
                          >
                            ...
                            <Button
                              ...
                              component="a"
                              href={`/api/returns/${data.id}/material-requisition/${req.id}/pdf`}
                              download={`${req.requisitionNumber}.pdf`}
                              sx={{
                                textTransform: 'none',
                                fontWeight: 600,
                                fontSize: '0.72rem',
          ...
          {STATUS_ORDER[activeUnit.status] >= 4 && !isScrapped && (
            <StageCard
              ...
              icon=""
              isComplete={returnStatus === 'REPAIR_COMPLETED'}
              isCurrent={
                activeUnit.status === 'REPAIR_COMPLETED' &&
                returnStatus !== 'REPAIR_COMPLETED'
              }
            >
              {returnStatus === 'REPAIR_COMPLETED' ? (
                <Alert severity="success" sx={{ borderRadius: 2 }}>
                  Repair approved and completed. Unit is ready for dispatch
                  processing.
                </Alert>
              ) : (
                <>
                  <Alert severity="info" sx={{ mb: 2, borderRadius: 2 }}>
                    All repair tasks have been completed. Awaiting production
                    manager approval to finalise repair.
                  </Alert>
                  {isManager && allUnitsRepairDone && (
                    <Button
                      variant="contained"
                      size="small"
                      startIcon={
      ...
      <Dialog
        ...
        onClose={() =>
          !stageLoading && setStageDialog((p) => ({ ...p, open: false }))
        }
        fullWidth
        maxWidth="sm"
      >
        ...
        <DialogContent dividers>
          ...
          <TextField
            ...
            onChange={(e) =>
              setStageDialog((p) => ({ ...p, notes: e.target.value }))
            }
            sx={{ mb: 2 }}
          />
          ...
          <FileUploadArea
            ...
            onAdd={(f) =>
              setStageDialog((p) => ({ ...p, files: [...p.files, ...f] }))
            }
            onRemove={(i) =>
              setStageDialog((p) => ({
                ...p,
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setAddTaskOpen(false)} disabled={addingTask}>
            Cancel
          </Button>
          <Button
            ...
            startIcon={
              addingTask ? <CircularProgress size={14} color="inherit" /> : null
            }
            sx={{
              bgcolor: '#0F172A',
      ...
      <Dialog
        ...
        onClose={() =>
          !completingTask && setCompleteDialog({ open: false, task: null })
        }
        fullWidth
        maxWidth="sm"
      >
        ...
        <DialogContent dividers>
          ...
          <FileUploadArea
            ...
            onRemove={(i) =>
              setTaskFiles((p) => p.filter((_, idx) => idx !== i))
            }
            onPreview={setPreviewImg}
      ...
      >
        ...
        <DialogContent dividers>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Search and add materials needed for this repair. The request will be
            sent to the inventory team.
          </Typography>
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <Box sx={{ flex: 1 }}>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        {item.material.partNumber} · {item.material.unit} ·{' '}
                        {item.material.currentStock} in stock
                      </Typography>
                    </Box>
                    <TextField
                      size="small"
                      ...
                      onChange={(e) => {
                        ...
                        setReqItems((prev) =>
                          prev.map((pi, i) =>
                            i === idx ? { ...pi, quantityRequired: v } : pi,
                          ),
                        );
                      }}
    ...
    >
      ...
      >
        ...
        <Typography
          ...
          fontWeight={700}
          sx={{
            color: isCurrent ? '#5B21B6' : isComplete ? '#15803D' : '#374151',
          }}
        >
          {icon} {title}
        </Typography>
            ...
            return (
              ...
              >
                ...
                >
                  {done ? (
                    <CheckCircleIcon
                      sx={{ color: '#16A34A', mt: 0.2, flexShrink: 0 }}
                    />
                  ) : (
